...
// Define file content type (matching your MessageBubble)
...
type Props = {
  initialValue: string;
  fileContents?: FileContent[]; // NEW: Add file contents prop
  onCancel: () => void;
...
export default function InlineEditor({
  initialValue,
  fileContents = [], // NEW: Default to empty array
  onCancel,
  onSave,
  className,
}: Props) {
  ...
  // File attachment component (same as MessageBubble)
  const FileAttachment = ({ file }: { file: FileContent }) => {
    ...
    return (
      <div className="">
        {/* <div className="relative isolate inline-flex max-w-full items-center gap-2 rounded-sm border border-[var(--border-heavy)] bg-[var(--bg-background-primary)] p-2 shadow-[0_0_0_1px_rgba(255,255,255,0.02)_inset]">
          ...
          <div className="min-w-0 leading-5">
            ...
            <div className="flex items-center gap-2 text-[11px] uppercase tracking-wide text-[var(--text-tertiary)]">
              <span>{file.mime?.split("/")[1] || "FILE"}</span>
              {file.size && (
                <>
                  <span>•</span>
  ...
  return (
    <div
      className={`w-full rounded-3xl  bg-[var(--bg-tertiary)] p-5 ${
        className ?? ""
      }`}
    >
      {/* Render file attachments first (just like ChatGPT) */}
      {fileContents.map((file, index) => (
        <FileAttachment key={index} file={file} />
      ))}

      <textarea
        className="w-full bg-transparent outline-none border-0 text-sm resize-none leading-3 max-h-[180px] overflow-y-auto text-[var(--text-primary)]"
        rows={3}
        value={value}
      ...
      <div className="mt-3 flex items-center gap-2 justify-end">
        <button
          type="button"
          onClick={onCancel}
          className="px-3.5 py-1.5 rounded-4xl text-sm text-[var(--text-secondary)] hover:bg-[var(--bg-elevated-primary)]  hover:border-gray-700 border border-[var(--bg-elevated-secondary)] transition-colors bg-[var(--bg-elevated-secondary)] cursor-pointer"
        >
          Cancel
        </button>
        <button
          ...
          className="px-[15px] py-[7px] rounded-4xl text-sm bg-[var(--interactive-bg-primary-default)] text-[var(--text-inverted)] cursor-pointer transition-colors"
        >
          Send
        </button>
      </div>
