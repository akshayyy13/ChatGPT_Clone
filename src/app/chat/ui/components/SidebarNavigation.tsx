...
export default function SidebarNavigation({
  ...
  const handleNewChat = async (e: React.MouseEvent) => {
    ...
    try {
      // ✅ Step 1: Create new thread via your API
      ...
      const { id: newThreadId } = await response.json();

      // ✅ Step 2: Refresh sidebar immediately (same as delete)
      refreshSidebar();

      // ✅ Step 3: Navigate directly to the NEW thread (not /chat)
      window.location.href = `/chat/${newThreadId}`;

      console.log("🆕 New chat created:", newThreadId);
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      >
        <button
          onClick={handleNewChat} // ✅ Use the corrected logic
          className={`w-full text-[13px] flex items-center  ${
            collapsed ? "" : "p-2  hover:bg-[#f9f9f9]/10"
          } rounded-lg  text-white/90 transition-colors flex-row justify-between group`}
        >
          <div
            className={`flex flex-row  ${
              collapsed
                ? "p-[8px] ml-0.5 w-10 rounded-xl hover:bg-[#f9f9f9]/10 "
                : ""
            } items-center`}
          >
            <svg
              ...
              className="icon items-center ml-0.5 "
              aria-hidden="false"
              aria-label="New chat"
            >
          ...
          <div
            className={`${headerContentVis} flex flex-row gap-0.5 opacity-0 group-hover:opacity-70`}
          >
            <p className=" text-[var(--text-secondary)] ">⇧</p>
            <p className=" text-[var(--text-secondary)] ">⌘</p>
            <p className=" text-[var(--text-secondary)] ">O</p>
          </div>
        </button>
      </Hover>

      {/* Rest of your navigation items remain unchanged */}
      <Hover
        text="Search chats"
        collapsed={collapsed}
        marginLeft="ml-20"
      ...
      >
        ...
        >
          ...
          >
            <p className=" text-[var(--text-secondary)]">⌘</p>
            <p className=" text-[var(--text-secondary)]">K</p>
          </div>
        </Link>
      </Hover>
      <Hover text="Library" marginLeft=" ml-16" marginTop="-mt-7" collapsed={collapsed}>
        <Link
          href="/chat/library"
          prefetch={false}
          className={`w-full text-[13px] flex items-center  ${
        ...
        >
          ...
          >
            <svg
              ...
              aria-label="Library"
            >
              <path d="M9.38759 8.53403C10.0712 8.43795 10.7036 8.91485 10.7997 9.59849C8.956 10.2819 10.4195 10.9133 9.73622 11.0096C9.05259 11.1057 8.4202 10.6298 8.32411 9.94614C8.22804 9.26258 8.70407 8.63022 9.38759 8.53403Z"></path>
              <path
                fillRule="evenodd"
                clipRule="evenodd"
      ...
      <div className="w-full h-[0.5px] mt-2 bg-[#fff1]"></div>
    </div>
  );
}
